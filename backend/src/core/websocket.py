...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, connection_type: str):
        ...
        logger.info(f"New WebSocket connection: user_id={user_id}, type={connection_type}")
    
    def disconnect(self, websocket: WebSocket, user_id: str, connection_type: str):
        """Disconnect a WebSocket client."""
        if (user_id in self.active_connections and 
            connection_type in self.active_connections[user_id]):
...
async def send_automation_update(automation_id: str, user_id: str, data: Dict[str, Any]):
